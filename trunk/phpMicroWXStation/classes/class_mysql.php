...
if (!extension_loaded('mysqli')) { error('Mysqli Extension not loaded.'); }

//Handles escaping
function db_string($String,$DisableWildcards=false) {
	global $DB;
	//Remove user input wildcards
	if ($DisableWildcards) { $String = str_replace(array('%','_'), '', $String); }
	//Escape and return
...
class DB_MYSQL {
	...
	function halt($Msg) {
		...
		//if ($this->Errno == 1194) { send_irc('PRIVMSG '.ADMIN_CHAN.' :@b@wht_red*** MySQL Error ***@b@wht_blk | '.$this->Error); }
